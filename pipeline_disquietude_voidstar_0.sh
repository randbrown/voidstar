#!/usr/bin/env bash
# pipeline_disquietude_voidstar_0.sh
#
...
INPUT_VIDEO_DEFAULT="~/WinVideos/disquietude_voidstar_0/disquietude_voidstar_0.mp4"
OUTDIR_DEFAULT="~/WinVideos/disquietude_voidstar_0"

# Highlight sampling defaults (leave start/full empty for divvy auto defaults).
# Disquietude shorter divvy targets sample from 1:23 onward.
START_SECONDS_DEFAULT="83"
...
TITLE_HOOK_TITLE_DEFAULT='// disquietude (hi60t)\n// voidstar'
...
GDRIVE_OUTDIR_DEFAULT="~/GoogleDrive/Music/voidstar/disquietude"   # e.g. /mnt/c/Users/<you>/Google Drive/My Drive/Videos mapped via symlink
...
main() {
    ...
    if [[ -z "${OUTDIR}" ]]; then
        if [[ -n "$OUTDIR_DEFAULT" ]]; then
            ...
            OUTDIR=$(eval echo "~/WinVideos/disquietude_voidstar_0")
        fi
    else
