#!/usr/bin/env bash
# pipeline_interactionism_voidstar_base.sh
#
...
INPUT_VIDEO_DEFAULT="~/WinVideos/interactionism/interactionism.mp4"
...
BASE_REELS_OVERLAY_PREBUILT_DEFAULT="~/WinVideos/interactionism/interactionism_reels_base_overlay.mp4"
...
PARTICLE_SPARKS_RATE_DEFAULT=0.75
PARTICLE_SPARKS_SIZE_DEFAULT=14
PARTICLE_SPARKS_LIFE_FRAMES_DEFAULT=60
PARTICLE_SPARKS_SPEED_DEFAULT=1.9
PARTICLE_SPARKS_JITTER_DEFAULT=1.3
PARTICLE_SPARKS_OPACITY_DEFAULT=0.9
PARTICLE_SPARKS_AUDIO_GAIN_DEFAULT=2.0
...
PARTICLE_SPARKS_SHOBUD_STROKE_WIDTH_DEFAULT=1.6
PARTICLE_SPARKS_SHOBUD_ROTATION_SPEED_DEFAULT=0.08
PARTICLE_SPARKS_SHOBUD_WOBBLE_DEFAULT=0.22
PARTICLE_SPARKS_SHOBUD_EDGE_SHIMMER_DEFAULT=0.35
PARTICLE_SPARKS_SHOBUD_EDGE_WIDTH_DEFAULT=1.2
...
TITLE_HOOK_LOGO_OPACITY_DEFAULT=0.25
...
GDRIVE_OUTDIR_DEFAULT="~/GoogleDrive/Music/voidstar/interactionism"   # e.g. /mnt/c/Users/<you>/Google Drive/My Drive/Videos mapped via symlink
...
# dvdlogo opacity tuning (conf-script tweakable).
DVDLOGO_OPACITY_DEFAULT=0.5
DVDLOGO_LOCAL_POINT_TRACK_OPACITY_DEFAULT=0.77
DVDLOGO_LOCAL_POINT_TRACK_LINK_OPACITY_DEFAULT=0.77
...
load_pipeline_config_file() {
    ...
    if [[ "$config_path" == ~* ]]; then
        eval "resolved=$config_path"
    elif [[ "$config_path" == /* ]]; then
        resolved="$config_path"
    else
        if [[ -f "$config_path" ]]; then
            resolved="$(readlink -f "$config_path" 2>/dev/null || echo "$config_path")"
        else
            resolved="$script_dir/$config_path"
        fi
    fi

    [[ -f "$resolved" ]] || die "Config file not found: $resolved"
    # shellcheck disable=SC1090
    source "$resolved"
    echo "[config] loaded: $resolved"
}
...
dvdlogo_cache_signature() {
    ...
    local source_clip="$2"
    local logo_path="$3"
    echo "dvdlogo|profile=${profile}|input=${source_clip}|input_fp=$(file_fingerprint "$source_clip")|logo=${logo_path}|logo_fp=$(file_fingerprint "$logo_path")|script=${DVDLOGO}|script_fp=$(file_fingerprint "$DVDLOGO")|opacity=${DVDLOGO_OPACITY}|track_opacity=${DVDLOGO_LOCAL_POINT_TRACK_OPACITY}|link_opacity=${DVDLOGO_LOCAL_POINT_TRACK_LINK_OPACITY}|voidstar_preset=${DVDLOGO_VOIDSTAR_PRESET}|voidstar_colorize=${DVDLOGO_VOIDSTAR_COLORIZE}"
}
...
write_project_notes_file() {
    ...
    cat > "$notes_path" <<EOF
// interactionism_${take_suffix} (original)
...
run_optional_particle_sparks_on_clip() {
    ...
    run_logged python3 "$PARTICLE_SPARKS" "$input_clip" \
        ...
        --shobud-render "$PARTICLE_SPARKS_SHOBUD_RENDER" \
        --shobud-stroke-width "$PARTICLE_SPARKS_SHOBUD_STROKE_WIDTH" \
        --shobud-rotation-speed "$PARTICLE_SPARKS_SHOBUD_ROTATION_SPEED" \
        --shobud-wobble "$PARTICLE_SPARKS_SHOBUD_WOBBLE" \
        --shobud-edge-shimmer "$PARTICLE_SPARKS_SHOBUD_EDGE_SHIMMER" \
        --shobud-edge-width "$PARTICLE_SPARKS_SHOBUD_EDGE_WIDTH" \
        --shobud-palette "$PARTICLE_SPARKS_SHOBUD_PALETTE" \
        --shobud-dark-suit-color "$PARTICLE_SPARKS_SHOBUD_DARK_SUIT_COLOR" \
        1>&2
...
run_60s_start() {
    ...
    if should_rebuild "$logo_stage" --dep "$source_for_logo" --dep "$logo" --dep "$DVDLOGO" --sig "$dvdlogo_sig"; then
        run_logged python3 "$DVDLOGO" "$source_for_logo" "$logo" \
            --speed 0 --logo-scale .4 --logo-rotate-speed 0 --trails 0.85 --opacity "$DVDLOGO_OPACITY" \
            --audio-reactive-glow 1.0 --audio-reactive-scale 0.5 --audio-reactive-gain 2.0 \
            --edge-margin-px 0 --reels-local-overlay false --voidstar-preset "$DVDLOGO_VOIDSTAR_PRESET" \
            ...
            --local-point-track-refresh 8 --local-point-track-opacity "$DVDLOGO_LOCAL_POINT_TRACK_OPACITY" --local-point-track-decay 0.77 \
            --local-point-track-link-neighbors 8 --local-point-track-link-thickness 1 \
            --local-point-track-link-opacity "$DVDLOGO_LOCAL_POINT_TRACK_LINK_OPACITY" --voidstar-colorize "$DVDLOGO_VOIDSTAR_COLORIZE" --start-x .8 --start-y .83 \
...
run_full() {
    ...
    if should_rebuild "$logo_stage" --dep "$source_for_logo" --dep "$logo" --dep "$DVDLOGO" --sig "$dvdlogo_sig"; then
        run_logged python3 "$DVDLOGO" "$source_for_logo" "$logo" \
            ...
            --local-point-track-refresh 8 --local-point-track-opacity "$DVDLOGO_LOCAL_POINT_TRACK_OPACITY" --local-point-track-decay 0.77 \
            --local-point-track-link-neighbors 8 --local-point-track-link-thickness 1 \
            --local-point-track-link-opacity "$DVDLOGO_LOCAL_POINT_TRACK_LINK_OPACITY" --voidstar-colorize "$DVDLOGO_VOIDSTAR_COLORIZE" --start-x .77 --start-y .79 \
            --content-bbox-for-local false --voidstar-debug-bounds true --voidstar-debug-bounds-mode hit-glitch \
            --voidstar-debug-bounds-hit-threshold 0.8 --voidstar-debug-bounds-hit-prob 0.1 \
            --output "$logo_stage"
            write_cache_signature "$logo_stage" "$dvdlogo_sig"
...
main() {
    local runtime_config="${INTERACTIONISM_PIPELINE_CONFIG:-${ATOMISM_PIPELINE_CONFIG:-}}"
    ...
    if [[ -n "$runtime_config" ]]; then
        load_pipeline_config_file "$runtime_config"
    fi
    ...
    PARTICLE_SPARKS_SHOBUD_RENDER="$PARTICLE_SPARKS_SHOBUD_RENDER_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_STROKE_WIDTH="$PARTICLE_SPARKS_SHOBUD_STROKE_WIDTH_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_ROTATION_SPEED="$PARTICLE_SPARKS_SHOBUD_ROTATION_SPEED_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_WOBBLE="$PARTICLE_SPARKS_SHOBUD_WOBBLE_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_EDGE_SHIMMER="$PARTICLE_SPARKS_SHOBUD_EDGE_SHIMMER_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_EDGE_WIDTH="$PARTICLE_SPARKS_SHOBUD_EDGE_WIDTH_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_PALETTE="$PARTICLE_SPARKS_SHOBUD_PALETTE_DEFAULT"
    PARTICLE_SPARKS_SHOBUD_DARK_SUIT_COLOR="$PARTICLE_SPARKS_SHOBUD_DARK_SUIT_COLOR_DEFAULT"
    ...
    if [[ -z "${OUTDIR}" ]]; then
        if [[ -n "$OUTDIR_DEFAULT" ]]; then
            ...
            OUTDIR=$(eval echo "~/WinVideos/interactionism")
        fi
    else
