#!/usr/bin/env bash
# pipeline_indistinguishable_particles_voidstar_cadebrown_0.sh
#
...
ENABLE_REELS_OVERLAY_STEP=0      # pose tracking disabled for this project; bypass reels overlay completely
...
GDRIVE_OUTDIR_DEFAULT="~/GoogleDrive/Music/voidstar/indistinguishable_particles_cadebrown"   # e.g. /mnt/c/Users/<you>/Google Drive/My Drive/Videos mapped via symlink
...
main() {
    ...
    if [[ -z "${OUTDIR}" ]]; then
        if [[ -n "$OUTDIR_DEFAULT" ]]; then
            ...
            OUTDIR=$(eval echo "/mnt/c/Users/brown/Videos")
        fi
    else
